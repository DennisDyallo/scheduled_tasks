...
function UpdateValues($root){
    Write-Host "We are in $root"

    $props = Get-ItemProperty $root
    
    foreach($prop in $props.PSObject.Properties.Where({-not $_.Name.StartsWith("PS")})){
        #Write-Host "Properties of root: $prop.Value"

        if($prop.Value.StartsWith("X:\")){
            #Write-Host "Setting item property of $prop.Value"  
            Set-ItemProperty -Path $root -Name $prop.Name -Value $prop.Value.Replace("X:\", "Z:\")
        }
    }
}
...
function SearchAndReplace { 
    ...
    Search-Registry -Path $Path -Recurse -SearchRegex "^X:" | ForEach-Object { UpdateValues $_.Key.PsPath}

}

SearchAndReplace -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Installer\UserData\*
